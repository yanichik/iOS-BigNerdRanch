...
//    }
//    for i in 0..<(len/2){
//        if brackets[String(sArr[i])] != String(sArr[len-i-1]){
//            return false
//        }
//    }
//    return true
...
//        "[": "]"
//    ]
//    let openings = brackets.keys
//    let closings = brackets.values
//    let sArr = Array(s)
//    let len = s.count
//    if len%2 != 0 {
//        return false
//    }
//    for i in 0..<len-1{
//        if openings.contains(brackets[String(sArr[i])]), closings.contains(brackets[String(sArr[i+1])]){
//            {
//                if brackets[String(sArr[i])] != String(sArr[i+1]){
//                    return false
//                } else {continue}
//            } else {continue}
//        }
...
func isValid(_ s: String) -> Bool {
    ...
    for i in 0..<len - 1 {
        if openings.contains(String(sArr[i])), closings.contains(String(sArr[i + 1])) {
            if brackets[String(sArr[i])] != String(sArr[i + 1]) {
                return false
            } else {
                continue
